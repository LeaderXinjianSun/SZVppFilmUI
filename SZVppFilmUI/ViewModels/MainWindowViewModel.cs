...
namespace SZVppFilmUI.ViewModels
{
    class MainWindowViewModel : NotificationObject
    {
        ...
        private void FindLineOperateCommandExecute(object p)
        {
            try
            {
                ...
                AddMessage("找到直线: " + (Math.Atan2(nc1.D, nr1.D) * 180 / Math.PI + 90).ToString("F2") + "°");
            }
            catch (Exception ex)
            {
    ...
    , out homMat2D);
        ...
        private void Init()
        {
            ...
            OnlyImage = true;
            string Station = Inifile.INIGetStringValue(iniParameterPath, "System", "Station", "A");
            WindowTitle = "SZVppFilmUI20200901_1:" + Station;
            TopCameraName = "cam3";
                    ...
                    switch (Station)
                    {
                        case "A":
                            ...
                            if (Fx5u.ReadM("M3100"))
                            {
                                ...
                                try
                                {
                                    ...
                                    if (rst)
                                    {
                                        ...
                                        if (isRecordImage != "0" && !calcrst.Item2)
                                        {
                            ...
                            if (Fx5u.ReadM("M3102"))
                            {
                                ...
                                try
                                {
                                    ...
                                    Task.Run(() => { rst2 = bottomCamera2.GrabImage(BottomCamera2Radius);
                                        if (rst2)
                                        {
                                            BottomCamera2Iamge = bottomCamera2.CurrentImage;
                        ...
                        case "B":
                            ...
                            if (Fx5u.ReadM("M3122"))
                            {
                                ...
                                {
                                    bool rst1 = false, rst2 = false, finish1 = false, finish2 = false;
                                    Task.Run(() => { rst1 = bottomCamera1.GrabImage(BottomCamera1Radius);
                                        if (rst1)
                                        {
                                            BottomCamera1Iamge = bottomCamera1.CurrentImage;
                                    ...
                                    Task.Run(() => {
                                        rst2 = bottomCamera2.GrabImage(BottomCamera2Radius);
                                        if (rst2)
                                        {
                                            BottomCamera2Iamge = bottomCamera2.CurrentImage;
        ...
        private Tuple<int[], bool> TopCameraCalc(string TargetD, double _x, double _y, double _u, int pindex)
        {
            try
            {
                ...
                double lineAngle1 = angle.TupleDeg().D;
                ...
                double lineAngle2 = angle1.TupleDeg().D;
                ...
                HTuple T1;
                HOperatorSet.VectorAngleToRigid(ProductP_x, ProductP_y, new HTuple(lineAngle1).TupleRad(), ProductP_x, ProductP_y, new HTuple(lineAngle2).TupleRad(), out T1);//T1是膜坐标旋转角度差的变换
                ...
                #region 范围
                bool result = true;
                if (lineAngle2 - lineAngle1 > 15 || lineAngle2 - lineAngle1 < -15)
                {
                    result = false;
                }
                #endregion
                ...
                //}                
                #endregion
                return new Tuple<int[], bool>(new int[3] { (int)(FitRobot_x0.D * 100 - targetp[0] + (CamImage_x1.D - CamImage_x.D) * 100), (int)(FitRobot_y0.D * 100 - targetp[1] + (CamImage_y1.D - CamImage_y.D) * 100), (int)((lineAngle2 - lineAngle1 + _u) * 100) }, result);
            }
            catch (Exception ex)
            {
                AddMessage(ex.Message);
