...
namespace SZVppFilmUI.ViewModels
{
    ...
    , out homMat2D);
        ...
        private void Init()
        {
            ...
            Product_DiffX1 = double.Parse(Inifile.INIGetStringValue(iniParameterPath, "System", "Product_DiffX1", "47.0362"));
        ...
        private Tuple<int[], bool> TopCameraCalc(string TargetD, double _x, double _y, double _u, int pindex)
        {
            try
            {
                ...
                {
                    if (FitRobot_y0.D * 100 - targetp[1] + (CamImage_y1.D - CamImage_y.D) * 100 * -1 > 5000 || FitRobot_y0.D * 100 - targetp[1] + (CamImage_y1.D - CamImage_y.D) * 100 * -1 < -5000)
                    {
                        result = false;
                    }
                    else
